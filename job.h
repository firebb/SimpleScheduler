/** @file job.h
 *  @brief This contains the structure that would express the pipeline DAG. 
 *
 *  Each operator maintains the input edges and output edges.
 */
#pragma once
...
class Operator {
 public:
  ...
  std::map<int, int> inOrder; // Contains the order for inputs.
...
class Job {
 public:
  ...
  Job(int numBatch) { this->numBatch = numBatch; }
